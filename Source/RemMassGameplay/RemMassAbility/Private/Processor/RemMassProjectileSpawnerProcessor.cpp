...
void URemMassProjectileSpawnerProcessor::Execute(FMassEntityManager& EntityManager,
    FMassExecutionContext& Context)
{
    ...
    EntityQuery.ForEachEntityChunk(Context, [&](FMassExecutionContext& Context)
    {
        ...
        for (auto Index = 0; Index < NumEntities; ++Index)
        {
            ...
            auto CanTrigger = [](const FRemMassProjectileTriggerInfoFragment& Info,
                const FRemMassProjectileTriggerStateFragment& State, const double Time)
            {
                // only one round, or max round reached
                if ((Info.RoundsPerInterval <= 1 || Info.RoundsPerInterval == State.CurrentRounds) && (
                        Info.ShotsPerRound <= 1 || Info.ShotsPerRound == State.CurrentShots))
                {
            ...
            auto IncrementShotCount = [](const FRemMassProjectileTriggerInfoFragment& Info,
                FRemMassProjectileTriggerStateFragment& State, const double Time)
            {
            ...
            const uint8 LoopCount = TriggerInfo.ShotsInterval > 0
                                        ? 1
                                        : TriggerInfo.ShotsPerRound;

            constexpr auto Degree     = 180.0f;
            ...
            const auto& ProjectileDirection = NearbyMonsterEntityData->NearbyMonsterDirections.IsEmpty()
                                                  ? OwnerRotation
                                                  : NearbyMonsterEntityData->NearbyMonsterDirections[0].Rotation().
                                                  Quaternion();
        ...
        if (SpawnDataContainer.ConfigAssets.Num() > 0)
        {
            ...
            Context.Defer().PushCommand<FMassDeferredCreateCommand>(
                [WeakRegenerator = TWeakObjectPtr<URemMassProjectileRegenerator>{EntityRegenerator}]
            (FMassEntityManager&)
                {
