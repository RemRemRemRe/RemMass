...
void URemMassDamageProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
    ...
    DamageTargetEntityQuery.ForEachEntityChunk(Context, [&](FMassExecutionContext& Context)
    {
        ...
        for (auto DamageSourceIndex = 0; DamageSourceIndex < DamageSourceNum; ++DamageSourceIndex)
        {
            for (auto EntityIndex = 0; EntityIndex < NumEntities; ++EntityIndex)
            {
                ...
                const auto DamageSourceLocation = DamageSourceTransformView[DamageSourceIndex].GetTransform().
                    GetLocation();
