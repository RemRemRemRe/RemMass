...
FAutoConsoleVariable CVarDisableRespawnRandomYawOffset
    {TEXT("Rem.Mass.Respawn.DisableRandomYawOffset"), false, TEXT("")};
    ...
    EntityQuery.ForEachEntityChunk(Context, [&](FMassExecutionContext& Context)
    {
        ...
        for (auto EntityIndex = 0; EntityIndex < NumEntities; ++EntityIndex)
        {
            ...
            const auto RespawnLocation = PlayerLocation +
                                         (CVarDisableRespawnRandomYawOffset->GetBool()
                                              ? ForwardDirection
                                              : RotatedForward) * RespawnRadiusView[EntityIndex].Value;
        ...
        Context.Defer().PushCommand<FMassDeferredCreateCommand>(
            [ExperienceTypeView, MovedTransformArray = std::move(SavedTransformArray)]
