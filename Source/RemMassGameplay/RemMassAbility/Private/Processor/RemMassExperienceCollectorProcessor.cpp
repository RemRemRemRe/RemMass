...
void URemMassExperienceCollectorProcessor::Execute(FMassEntityManager& EntityManager,
    FMassExecutionContext& Context)
{
    ...
    EntityQuery.ForEachEntityChunk(Context, [&](FMassExecutionContext& Context)
    {
        ...
        for (auto& PlayerEntityHandle : GameStateSubsystem.GetPlayerEntityView())
        {
            ...
            const auto ExpCollectRadiusSquared = FMath::Square(
                PlayerEntityView.GetFragmentData<FRemMassExpCollectRadiusFragment>().Value);
            ...
            for (auto EntityIndex = 0; EntityIndex < NumEntities; ++EntityIndex)
            {
                    ...
                    PlayerExperience.Value >= LevelUpExperience.Value)
                {
                    ...
                    LevelUpExperience.Value = static_cast<int32>(Curve->Eval(PlayerLevel.Value,
