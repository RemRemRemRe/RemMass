    ...
    const UWorld& World) const
{
    ...
    BuildContext.GetMutableObjectFragmentInitializers().Add(
        [&World, this](UObject& Object, const FMassEntityView& EntityView, const EMassTranslationDirection)
        {
            ...
            EntityManager.Defer().PushCommand<FMassDeferredCreateCommand>(
                [this, &World, &Object, OwnerHandle = EntityView.GetEntity()]
            (const FMassEntityManager& Manager)
                {
                    ...
                    for (const auto& Value : InitialItems)
                    {
                        ...
                        for (auto Index = 0; Index < Handles.Num(); ++Index)
                        {
                        ...
                        InitialItemEntities.FindOrAdd(InitialItemEntitiesPair.Key,
                            std::move(InitialItemEntitiesPair.Value));
                    }
